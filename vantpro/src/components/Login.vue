    ...
    <div>
        ...
        <van-tab title="注册" style="text-align: center;">
            ...
            <van-field v-model="upwd"  placeholder="请输入密码" >
                <template #button>
                    <van-switch v-model="checked" active-color="rgb(0,166,124)"/>
                </template>
            </van-field>
...
<script>
import { Field ,NavBar,Button ,Notify ,Switch } from 'vant';
...
export default {
    ...
    methods: {
        ...
        LoginBtn:function(){
            //账号 admin 密码 123456
            var that = this;
            this.$http.post('/user/login',{"account":this.uid,"password":this.upwd}).then(function(res){
